...
def _dataframe_to_coo_sparse(df: pd.DataFrame, dtype=np.float32) -> sp.coo_matrix:
    """Return a COO matrix without densifying; cast to pandas-Sparse if needed."""
    ...
    if not any(isinstance(dt, pd.SparseDtype) for dt in df.dtypes):
        ...
        # Making sure that subtype matches the desired dtype.
        df = df.astype(pd.SparseDtype(np.dtype(dtype), fill), copy=False)
...
def _pad_and_cast(
    ...
    if not isinstance(desired_dtype, pd.SparseDtype):
        raise TypeError("The variable desired_dtype must be a pandas SparseDtype")
...
def _pl_pos_to_pd(tbl: pl.DataFrame) -> pd.DataFrame:
    """Convert polars DataFrame to pandas DataFrame. Assumes only integers/floats are present."""
    df_pd = tbl.to_pandas()
...
def _df_to_csc_sparse_v2(df: pd.DataFrame, dtype=np.float32) -> sp.csc_matrix:
    """Convert Dense or pandas-Sparse DataFrame (variants x cells) to CSC (cells x variants) without densifying."""
    # We check if the dtype is sparse.
    if any(isinstance(dt, pd.SparseDtype) for dt in df.dtypes):
        coo = df.sparse.to_coo()  # variants x cells ==> transposing back to cells x variants
...
def _df_to_csc_sparse(df: pd.DataFrame, dtype=np.float32) -> sp.csc_matrix:
    """
    Convert a (variants x cells) DataFrame to a CSC matrix (cells x variants)
    ...
    # Sparse conversion: variants x cells (COO) -> CSC, then transpose to cells x variants
...
# Helper converting Polars to Pandas float64
...
def _compute_weighted_qualities(
    ...
    *,
    ...
    Returns a dictionary with:
      - alt_qual_global,      ref_qual_global,      all_qual_global     : DataFrame (variants x cells, float32)
    ...
    # Denominators (counts) alternative allele
    if alt_reads is None:
        alt_reads = (alt_fwd + alt_rev).astype(np.int32, copy=False)

    # Denominators (counts) reference allele
    ...
    # Per-variant x per-cell weighted qualities
...
def Load_VarTrix_typewise(
    ...
    *,
    ...
    -- type_use: What type of experiment should be loaded? If a patient has
                 ...
                 loaded.
    -- min_reads: The minimum number of reads neeeed
    ...
    -- cells_include: A set of cells. Only these cells will be included. All
                      others will be removed. These barcodes need to have the
                      correct patient prefix.
    -- cells_exclude: A set of cells. These cells will be removed and the rest
                      retained. These barcodes need to have the correct patient prefix.
    ...
    if cells_include is not None:
        ...
        col_labels = [c for c, k in zip(col_labels, mask, strict=True) if k]
    ...
    # Question when you come back: why don't I zero alt reads here?
    # Answer: the alt reads are the coverage. See further down below.
    # So, we only need to zero the coverage and the reference, because 'coverage' is actually the alternative reads.
...
def LoadingMGATK_typewise(
    ...
    if alt_fwd_global.empty or alt_fwd_global.shape[1] == 0:
        if verbose:
            print(
                f"Filtering left {alt_fwd_global.shape[0]} variants x {alt_fwd_global.shape[1]} cells; returning None.",
                flush=True,
            )
...
def LoadingMAEGATK_typewise_visiumHD(
    ...
    *,
    ...
    Visium HD loader (chunk-merge aware):
        - A Visium experiment has too many barcodes to be analyzed by MAEGATK.
        - Splitting into chunks based on the barcodes.
        ...
        - This keeps memory low and allows MAEGATK to function.
    ...
    if verbose:
        print("Position repeated 3x (one per alt != ref).", flush=True)
    ...
    mut_idx_pl = pl.DataFrame({"mutation": muts.tolist(), "mut_idx": np.arange(n_vars, dtype=np.int32)})
    if verbose:
        print("Position -> mut_idx (position appears 3x across mut_idx).", flush=True)

    pos_to_mut_pl = pl.DataFrame({"position": mut_positions, "mut_idx": np.arange(n_vars, dtype=np.int32)})
    ...
    for sample_use, grp in sample_groups:
        ...
        map_paths = (
            [Path(parquet_cell_map_path)] if parquet_cell_map_path is not None else [Path(p) for p in parquet_paths]
        )
        ...
        cell_idx_pl = pl.DataFrame({"cell_id": cell_ids, "cell_idx": np.arange(n_cells, dtype=np.int32)})

        # accumulators (cells x variants)
    ...
    alt_qual_global = _safe_div_sparse(alt_qsum, alt_reads)  # cells x vars
