...
def Filtering(
    ...
    if (min_cells_per_variant is not None) and ((not np.isfinite(min_cells_per_variant)) or (min_cells_per_variant < 0)):
    ...
    if (min_variants_per_cell is not None) and ((not np.isfinite(min_variants_per_cell)) or (min_variants_per_cell < 0)):
...
def _scale_coords_to_um(xy: np.ndarray, microns_per_pixel = None) -> np.ndarray:
    ...
    if isinstance(microns_per_pixel, (bool, np.bool_)) or not isinstance(microns_per_pixel, numbers.Real) or microns_per_pixel <= 0:
...
def build_knn_neighborlists(xy: np.ndarray, k: int, max_distance = None) -> list[np.ndarray]:
    ...
    if max_distance is not None:
        if isinstance(max_distance, (bool, np.bool_)) or not isinstance(max_distance, numbers.Real) or max_distance <= 0:
...
def Pruning(
    ...
    *,
    ...
    knn_max_distance_um: float = None,
    ...
    if isinstance(min_pos_neighbors, (bool, np.bool_)) or not isinstance(min_pos_neighbors, numbers.Integral) or min_pos_neighbors < 0:
    ...
    if neighbor_mode == "hops":
        ...
        if isinstance(n_neighbors, (bool, np.bool_)) or not isinstance(n_neighbors, numbers.Integral) or n_neighbors < 1:
            raise ValueError("n_neighbors must be an integer >= 1.")
        
        if knn_max_distance_um is not None:
            if isinstance(knn_max_distance_um, (bool, np.bool_)) or not isinstance(knn_max_distance_um, numbers.Real) or knn_max_distance_um <= 0:
