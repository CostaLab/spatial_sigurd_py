...
def test_all_three_codes_and_variant_precedence():
    alt, cov = _codes()
...
def test_output_stays_sparse():
    alt, cov = _codes()
    assert sp.issparse(_build_consensus(alt, cov))


def test_nocall_positions_not_stored():
    alt, cov = _codes()
    out = _build_consensus(alt, cov)
...
def test_default_dtype_is_int8():
    alt, cov = _codes()
    assert _build_consensus(alt, cov).dtype == np.int8


def test_custom_dtype_respected():
    alt, cov = _codes()
    assert _build_consensus(alt, cov, dtype = np.int16).dtype == np.int16


def test_stored_explicit_zero_in_alt_is_not_a_variant():
...
def test_no_sparse_efficiency_warning():
    alt, cov = _codes()
